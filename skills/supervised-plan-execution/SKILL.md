...
## Retroactive Plan Compatibility

Plans generated by earlier versions of `writing-plans` may contain baked-in commit instructions that duplicate or mis-specify the temp file pattern (e.g., missing the Read step).

**When executing commit steps from any plan, always follow `core-commands` for the full temp file sequence.** This supersedes any commit instructions embedded in the plan document — those may be outdated. `core-commands` is the single source of truth.

If a plan's commit step conflicts with `core-commands`, follow `core-commands`.
